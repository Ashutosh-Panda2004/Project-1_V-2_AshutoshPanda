...
const InputForm = ({ setEvaluationResult }) => {
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        console.log('Submitting values:', values); // Debugging line
        const response = await axios.post('https://project-1-v-2-ashutoshpanda.onrender.com/api/rules/evaluate', values);
        console.log('Response:', response.data); // Debugging line
  ...
  return (
    <form onSubmit={formik.handleSubmit} className="max-w-md mx-auto bg-gray-800 p-6 rounded-lg">
      ...
      {/* Experience Input */}
      <div className="mb-4">
        <label className="block text-white mb-1" htmlFor="experience">Experience (Years)</label>
        <input
          id="experience"
          name="experience"
          type="number"
